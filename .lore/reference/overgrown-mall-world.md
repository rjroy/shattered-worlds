...
Spore is a player card with `energyCost: 1`, `exhaust: true`, `effect: None`, and the `Spore` keyword. Playing it is pruning: it costs tempo but removes the pollution from the run. Bloom uses `DealProgressScaled` to reward carrying Spores, making "clean it out" and "grow with it" both live strategic lines.
