...
- **§4 / REQ-EMBER-26/47:** Hatchery Cellar `onCleared` runs `OfferBoon { setId: "ember-boons", offeredCount 3, chooseCount 1 }` instead of `Sequence[GainCard ×5]`. Rewards single-sourced in `src/data/worlds/boons/ember.json` (A8).
...
- [~] **Slice C — Tests and validation** (C1-C6) — LOGIC TESTS DONE, art/smoke deferred
  ...
  - [x] C2 incubation-pattern effect tests (OfferBoon-on-clear from ember-boons, ForceDestroy, Brace absorption, Hatchery top-deck order)
...
- 2026-06-21: A8 revision done — Keep Vigil + Bank the Heat added; ember-boons now 5 tools, Hatchery offers 3-of-5. Tests still 1180 pass / 2 expected fails.
