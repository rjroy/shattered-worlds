...
- REQ-EVENTS-8: The run-end envelope must identify the terminal outcome of the session and must close the stream exactly once per session. At minimum, it must include the same session identifier, the terminal outcome (`won` or `lost`), and the final act reached. A session that ends in victory and a session that ends in loss must be distinguishable to subscribers without inferring outcome from renderer behavior.
