...
## Keyword and closer resolution (2026-07-03)

Checking the World 13/14/15 mechanic ideas against `.lore/reference/worlds/authoring/theme-authoring.md` surfaced two contract conflicts, both now resolved. Full detail lives in the per-world docs (`world-13-grief-mechanics.md`, `world-14-grief-mechanics.md`, `world-15-grief-mechanics.md`); summary here for the run-length view:

- **Freeze/Thaw/Heat is `whiteout-parking-garage`'s signature suite**, and the original mechanic drafts leaned on it as the connective tissue across all three worlds. Dropped as the spine; kept only as optional minor flavor. Replaced with five new applied keywords carrying the actual weight: `Denial` + `Anger` (World 13), `Bargaining` + `Depression` (World 14), reprised together in World 15 Act I/II, plus `Acceptance` (World 15 Act III only). All five are registered in `KEYWORD_COST_MODIFIERS` — the first four tax cost, `Acceptance` is the one keyword in the game that reduces it.
- **World 15's "win condition is not the Door" directly conflicted with D1** (Act 3 always ends with `The Walker` → `Door` → `SurviveWorld`). Resolved by introducing `Destiny` as its own themed entity (not a shared/unthemed template like Walker or Door): in 13 and 14, Destiny is the trouble — a hazard whose cost scales with grief-keyword saturation — while Door and Walker keep their standard, unmodified roles. In 15, `Door` and `Walker` stay exactly as standard as they are everywhere else — no special-casing, no added tax, the ordinary flight instinct is still sitting there untouched. `Destiny` is authored with a base cost high enough to be nigh-impossible to clear without substantial accumulated `Acceptance`; as Acceptance stacks, its cost drops until it's reachable, and its own `onCleared` fires `SurviveWorld` — an earned alternate path, not a change to Door's difficulty. Nothing here required an engine change or touched a shared template.
