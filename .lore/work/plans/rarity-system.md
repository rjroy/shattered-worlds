...
related: [.lore/work/specs/rarity-system.md, .lore/work/brainstorm/rarity-system.md, .lore/work/specs/offer-boon-rewards.md, .lore/work/specs/fortune-boon-cards.md]
---
...
- **Fixture regeneration (REQ-RARITY-22):** offered IDs and all downstream RNG draws change for seeds that hit an offer. The seed-pinned offer fixtures live in `src/core/tests/reduce.test.ts` — it hardcodes literal offered-ID arrays for specific seeds (`seed777Offer`, `seed778Offer` around line 1977, and standalone `offeredTemplateIds: [...]` expectations at lines ~2009, 2454, 2524, 2603, 2643, 2664). Regenerate each by running the seed, capturing the actual offered IDs, and replacing the literal arrays — never hand-patch values. Treat a same-seed twice-identical result as the determinism proof. Note: `golden.test.ts` (Door-win, Zombie-loss, replay-equivalence) does **not** touch `createBoonOffer` and should remain green untouched; if it diverges, that is a signal something broke beyond offer composition.
...
- `src/core/model/types.ts`:
  - `BoonOffered`: add `rarities: readonly RarityTier[]` to **both** union arms (`act` and `worldClear`), index-aligned with `templateIds`, preserving the `source`/`act` discriminated split exactly (REQ-RARITY-32, 32a).
...
**Requirements:** REQ-RARITY-24, 25, 26, 27, 28, 29, 31. Implements D1.
...
- `src/core/model/types.ts`: add to `CardEffect` union:
  ```typescript
  | { kind: "GainRandomCard"; setId: string; bToDiscard?: boolean }
  ```
  (Default destination is `playerDiscard` to match `GainCard`; `bToDiscard` optional for future flexibility — REQ-RARITY-26.)
- `src/core/effects/gainCard.ts`: add `GainRandomCardHandler`:
  - `apply`: resolve pool → filter legal candidates → `weightedDraw(catalog, state.rng, candidates, 1)` → `gainCard(catalog, state, drawnId, "playerDiscard")`. The roll happens here at resolution/mint time and the concrete card + tier are revealed via the `CardGained` event (REQ-RARITY-25).
...
- Point exactly one *generic-reward, cache-style* `onCleared` at `{ "kind": "GainRandomCard", "setId": "<pool>" }`. Candidate target: a non-identity reward in an existing world (review `src/data/worlds/*/cards.json` `onCleared` `GainCard` entries; pick one whose specific card is not load-bearing world identity). Do **not** convert `Fire Axe`/`Nitro`-class identity loot (D2).
...
- Spot-check in the running app (per project "fun + correct" gate): clear the Step-7 cache card and a Fortune act transition, confirm rarity colors render on offers and grants.
