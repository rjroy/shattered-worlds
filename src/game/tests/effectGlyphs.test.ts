...
describe('compileEffect', () => {
  ...
  it('compiles all four Draw variants', () => {
    ...
    expect(compileEffect({ kind: 'Draw', world: 1 })).toStrictEqual([line([i('worldDraw'), v('+1')])])
  ...
  it('compiles the remaining simple player effects', () => {
    ...
    expect(compileEffect({ kind: 'AddCard', template: 'Listen', dest: 'playerDiscard' })).toStrictEqual([
      line([i('addCard'), t('Listen')]),
    ])
  })
  ...
  it('compiles the hazard effect kinds', () => {
    ...
    expect(compileEffect({ kind: 'AddThreatToWorldDeck' })).toStrictEqual([line([i('threat'), v('+1')])])
    expect(compileEffect({ kind: 'SkipDrawNextTurn' })).toStrictEqual([
      line([i('skipDraw'), t('next turn')]),
    ])
    expect(compileEffect({ kind: 'SurviveWorld' })).toStrictEqual([line([i('survive'), t('survive')])])
