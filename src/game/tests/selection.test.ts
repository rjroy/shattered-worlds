...
describe('Cut It Loose walk (destroyHand(1,1) + hazard)', () => {
  ...
  it('advance after destroyHand pick moves to hazard step', () => {
    ...
    sel = advance(sel)
    if (sel.phase !== 'targeting') throw new Error('expected targeting')
    expect(sel.done[0]).toEqual({ kind: 'destroyHand', destroyId: 'p2' })
    expect(sel.stepIdx).toBe(1)
    expect(sel.current).toHaveLength(0)
  })
...
describe('needsConfirm boundaries', () => {
  ...
  it('destroyHand(0,2): advance with 0 picks → buildAction omits destroyId', () => {
    const spec: TargetSpec = { kind: 'destroyHand', min: 0, max: 2 }
    let sel = beginTargeting('p1', spec)
    sel = advance(sel)
    expect(isComplete(sel)).toBe(true)
    ...
    expect(action).not.toHaveProperty('destroyId')
    expect(action).toEqual({ type: 'PlayCard', cardId: 'p1' })
  })

  it('destroyHand(0,2): advance with 2 picks → destroyId set (first pick used)', () => {
    const spec: TargetSpec = { kind: 'destroyHand', min: 0, max: 2 }
    let sel = beginTargeting('p1', spec)
    sel = togglePick(sel, 'p2')
    sel = togglePick(sel, 'p3')
    sel = advance(sel)
    // destroyHand max is 2 but buildAction uses done[0].destroyId which is p2
    const action = buildAction(sel)
    expect(action).not.toBeNull()
    // We only store current[0] as destroyId per the spec
    expect((action as Record<string, unknown>).destroyId).toBe('p2')
  })
})
...
describe('hintForSelection', () => {
  ...
  it('hazard step → "Select a Hazard target"', () => {
    ...
    expect(hintForSelection(sel)).toEqual({ text: 'Select a Hazard target', visible: true })
  })
