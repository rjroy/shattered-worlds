...
describe("describeEffect Brace", () => {
  it("describes a single-charge Brace with singular phrasing", () => {
    expect(describeEffect({ kind: "Brace", amount: 1 })).toEqual(["Brace: absorb the next snatch"]);
  });
...
describe("previewPlay", () => {
  ...
  it("reports remaining Progress and counts what was already dealt this turn", () => {
    ...
    expect(previewPlay(explore, walker, makeState())).toBe(
      "Make 1 Progress → 9 more to clear The Walker",
    );
