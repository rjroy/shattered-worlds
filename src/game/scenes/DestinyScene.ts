...
export class DestinyScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.input.on('wheel', (pointer: Phaser.Input.Pointer, _over: unknown, _dx: number, deltaY: number) => {
      if (pointer.y < GRID_TOP || pointer.y > GRID_TOP + VISIBLE_ROWS * (CARD_H + GRID_GAP_Y)) return
  ...
  private render(): void {
    ...
    if (this.featsStore === undefined || this.unlocksStore === undefined) {
      this.addToContent(this.add.text(CANVAS_W / 2, 300, 'Destiny is quiet.', textStyle({
        fontSize: '22px',
    ...
    this.addPanel(44, 78, 812, 28)
    this.addText(64, 84, `✦ ${balance} Fragments`, 14, TEXT.textReward, true)
    this.addText(600, 84, `Destiny ${pips} ${used}/${DESTINY_BUDGET}`, 14, '#d6b15c', true)

    const rows = Math.ceil(UNLOCK_CATALOG.length / 2)
    const maxOffset = Math.max(0, rows - VISIBLE_ROWS)
    this.scrollOffset = Math.min(this.scrollOffset, maxOffset)
    const visible = UNLOCK_CATALOG.slice(this.scrollOffset * 2, (this.scrollOffset + VISIBLE_ROWS) * 2)

    visible.forEach((def, index) => {
      const col = index % 2
      const row = Math.floor(index / 2)
      this.renderCard(def, GRID_LEFT + col * (CARD_W + GRID_GAP_X), GRID_TOP + row * (CARD_H + GRID_GAP_Y), balance, profile)
    })

    if (maxOffset > 0) {
      this.addText(780, 84, `${this.scrollOffset + 1}-${Math.min(this.scrollOffset + VISIBLE_ROWS, rows)} of ${rows}`, 11, TEXT.textMuted)
      if (this.scrollOffset > 0) {
        const up = this.add.text(842, GRID_TOP, '▲', textStyle({ fontSize: '13px', color: '#d6b15c' }))
          .setInteractive({ useHandCursor: true })
          .on('pointerdown', () => this.scrollBy(-1))
        this.addToContent(up)
      }
      if (this.scrollOffset < maxOffset) {
        const down = this.add.text(842, GRID_TOP + VISIBLE_ROWS * (CARD_H + GRID_GAP_Y) - 28, '▼', textStyle({ fontSize: '13px', color: '#d6b15c' }))
          .setInteractive({ useHandCursor: true })
          .on('pointerdown', () => this.scrollBy(1))
  ...
  private renderCard(
    ...
    if (this.textures.exists(artKey)) {
      ...
      card.add(this.add.text(50, 42, '✦', textStyle({ fontSize: '22px', color: '#d6b15c' })).setOrigin(0.5, 0.5))
    }

    card.add(this.add.text(102, 14, def.name, textStyle({
      fontSize: '15px',
      color: TEXT.textWorldTitle,
      fontStyle: 'bold',
      wordWrap: { width: 190 },
    })))
    card.add(this.add.text(314, 15, '●'.repeat(def.destinyWeight), textStyle({
      fontSize: '13px',
      color: '#d6b15c',
    })))
    card.add(this.add.text(102, 40, effectSummary(def), textStyle({
      fontSize: '12px',
      color: TEXT.textReward,
      wordWrap: { width: 246 },
    })))
    card.add(this.add.text(102, 62, def.description, textStyle({
      fontSize: '11px',
      color: TEXT.textMuted,
      fontStyle: 'italic',
      wordWrap: { width: 246 },
    })))
    card.add(this.add.text(102, 96, `✦ ${def.cost}`, textStyle({
      fontSize: '12px',
    ...
    if (state === 'owned') {
      card.add(this.add.text(170, 96, '✓ owned', textStyle({
        fontSize: '12px',
        color: TEXT.textReward,
        fontStyle: 'bold',
      })))
      ...
      card.add(this.add.text(286, 96, 'locked', textStyle({
        fontSize: '12px',
        color: TEXT.textMuted,
      })))
    }
  ...
  private addActivationToggle(
    ...
    const toggle = this.add.text(274, 96, label, textStyle({
      fontSize: '12px',
      color,
      fontStyle: active ? 'bold' : '',
    }))
    if (enabled) {
      toggle.setInteractive({ useHandCursor: true }).on('pointerdown', () => this.toggleActive(def, !active))
    } else {
  ...
  private toggleActive(def: UnlockDefinition, active: boolean): void {
    ...
    if (result !== 'ok') {
      this.messageText?.setText(result === 'over-budget' ? 'The thread will not hold more weight.' : 'That Blessing is not owned.')
    } else {
  ...
  private confirmPurchase(def: UnlockDefinition): void {
    ...
    panel.setRounded(8)
    const body = this.add.text(0, -52, `Buy ${def.name} for ✦ ${def.cost}?`, textStyle({
      fontSize: '18px',
      color: TEXT.textLight,
      align: 'center',
      wordWrap: { width: 430 },
  ...
  private createCardButton(x: number, y: number, label: string, onClick: () => void): Phaser.GameObjects.Container {
    const container = this.add.container(x, y)
    const bg = this.add.rectangle(0, 0, 76, 28, 0x0d0a12, 0.95)
    bg.setStrokeStyle(1, 0xd6b15c, 0.9)
    bg.setRounded(6)
    bg.setInteractive({ useHandCursor: true })
    const text = this.add.text(0, -7, label, textStyle({
      fontSize: '12px',
      color: '#d6b15c',
      fontStyle: 'bold',
    })).setOrigin(0.5, 0)
