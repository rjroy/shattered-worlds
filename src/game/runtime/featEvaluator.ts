...
function resolveStat(statId: string, ctx: EvaluationContext): unknown {
  ...
  if (statId.startsWith('world.')) {
    const field = statId.slice('world.'.length)
    const worldEntry = ctx.lifetime.byWorld[ctx.run.worldId]
  ...
  if (statId.startsWith('witness.')) {
    ...
    return (ctx.witness.threats[templateId] as unknown as Record<string, unknown> | undefined)?.[field]
  }
...
export function createFeatEvaluator(
  ...
  const subscriber: RunStreamSubscriber = (item: RunStreamItem) => {
    ...
    if (lifetime.lastRun === undefined) {
      console.warn('[featEvaluator] RunEnded received but lifetime.lastRun is undefined; skipping feat evaluation')
      return
    }
    ...
    for (const definition of catalog) {
      ...
      if (evaluateFeat(definition, ctx)) {
        featsStore.appendFeat({ featId: definition.id, earnedAt: clock(), sessionId: item.sessionId })
