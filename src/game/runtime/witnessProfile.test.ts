...
describe("createWitnessCollector", () => {
  it("AI #7: tracks encounter count and diedTo for a lost run with hazard in hand", () => {
    ...
    expect(profile.threats["Zombie"]).toEqual({
      encounterCount: 2,
      resolvedCount: 0,
      discardedCount: 0,
      diedTo: true,
    ...
    expect(reloaded.threats["Zombie"]).toEqual({
      encounterCount: 2,
      resolvedCount: 0,
      discardedCount: 0,
      diedTo: true,
  ...
  it("accepts a well-formed witness profile", () => {
    const profile: WitnessProfile = {
      version: 1,
      threats: {
        Zombie: { encounterCount: 3, resolvedCount: 0, discardedCount: 0, diedTo: true },
        Screams: { encounterCount: 1, resolvedCount: 0, discardedCount: 0, diedTo: false },
      },
    };
  ...
  it("setProfile persists the new profile to storage", () => {
    ...
    const newProfile: WitnessProfile = {
      version: 1,
      threats: {
        Rubble: { encounterCount: 5, resolvedCount: 0, discardedCount: 0, diedTo: false },
      },
    };
