...
function recordLostRun(
  collector: ReturnType<typeof createRunStatsCollector>,
  sessionId: string,
): void {
...
describe("statsTransfer", () => {
  it("round-trips exported lifetime and history into a fresh collector", () => {
    ...
    expect(JSON.parse(targetStorage.dump()[RUN_HISTORY_STORAGE_KEY]!)).toEqual({
      version: 1,
  ...
  it("rejects invalid imports without mutating in-memory or stored stats", () => {
    ...
    const inspected = transfer.inspectImport(
      JSON.stringify({ kind: "shattered-worlds-stats", lifetime: { version: 99 } }),
    );
  ...
  it("exportJson includes witness and feats profiles when non-empty", () => {
    ...
    expect(
      (parsed.witnessProfile as { threats: Record<string, { encounterCount: number }> }).threats[
        "Zombie"
      ]!.encounterCount,
    ).toBe(1);
  ...
  it("a full payload with both profiles replaces both on applyImport", () => {
    ...
    const importedWitness = {
      version: 1 as const,
      threats: { Dragon: { encounterCount: 5, resolvedCount: 0, discardedCount: 0, diedTo: true } },
    };
    const importedFeats = {
      version: 1 as const,
      earned: [{ featId: "slew-the-dragon", earnedAt: 9000, sessionId: "epic" }],
    };
