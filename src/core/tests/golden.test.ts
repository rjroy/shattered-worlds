...
describe("replay equivalence: Spore + DealProgressScaled", () => {
  ...
  function runReplay(): {
    ...
    const spore = r2.state.hand.find(
      (c): c is PlayerCard => c.kind === "player" && c.name === "Spore",
    );
    const drawnBloom = r2.state.hand.find(
      (c): c is PlayerCard => c.kind === "player" && c.name === "Bloom",
    );
    const target = r2.state.hand.find(
      (c): c is WorldCard => c.kind === "world" && c.name === "Strange Sounds",
    );
    ...
    return {
      state: r4.state,
      eventTypes,
      topDeckGains: allEvents.filter((e) => e.type === "CardGained" && e.dest === "playerDrawTop")
        .length,
