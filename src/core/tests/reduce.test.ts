...
describe("ForceDestroy onDiscarded", () => {
  ...
  it("the queued charge destroys a player card from the refilled hand on EndTurn", () => {
    ...
    expect(afterEnd.state.hand.some((c) => destroyedIds.includes(c.id))).toBe(false);
...
describe("post-terminal throws", () => {
  it("any action after WorldLost throws IllegalActionError", () => {
    ...
    expect(() => reduce(catalog, lostState, { type: "DiscardHazard", cardId: zombie.id })).toThrow(
      IllegalActionError,
    );
  });
...
describe("EndTurn Corpse self-transform", () => {
  ...
  it("an undealt Corpse degrades: gone from hand and a Zombie is spawned onto the world deck", () => {
    ...
    const worldCardsAfter =
      result.state.worldDraw.length + result.state.hand.filter((c) => c.kind === "world").length;
    expect(worldCardsAfter).toBe(beforeWorldDraw + 1);

    // The spawned world card is a Zombie (it was drawn into the refilled hand).
    const spawnedZombie = result.state.hand.find((c) => c.kind === "world" && c.name === "Zombie");
...
describe("Boon offer generation", () => {
  ...
  it("creates one queued offer per ActAdvanced when one refill advances multiple acts", () => {
    ...
    expect(boonEvents).toHaveLength(2);
  ...
  it("multiple real act transitions create one choice each without duplicate offers", () => {
    ...
    expect(first.state.pendingBoonChoices).toHaveLength(1);
  ...
  it("filters invalid duplicate non-player and non-exhaust boon templates", () => {
    ...
    expect(result.state.pendingBoonChoices[0]?.offeredTemplateIds).toEqual(offered);
  });
  ...
  it("rescues zero-player-draw act transitions until ChooseBoon resolves", () => {
    ...
    expect(activeResult.state.pendingBoonChoices).toHaveLength(1);
