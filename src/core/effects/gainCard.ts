import type { CardEffect, CardTemplateId, Dest, GameEvent, GameState, WorldCard } from '../model/types'
...
export class AddWorldCardToDeckHandler extends EffectHandler<AddWorldCardToDeckEffect> {
  override apply(ctx: EffectContext, effect: AddWorldCardToDeckEffect): EffectResult {
    return gainCard(ctx.catalog, ctx.state, effect.template, effect.bTop ? 'worldDrawTop' : 'worldDraw')
  }
