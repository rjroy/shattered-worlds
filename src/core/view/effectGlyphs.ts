...
function compile(effect: CardEffect, ctx: CompileContext): EffectLine[] {
  switch (effect.kind) {
    case 'DealProgress': {
      const lines = [main([icon('progress'), value(`${effect.base}`, 'progress')])]
      if (effect.bonus) lines.push(bonusRider(effect.bonus))
      return lines
    }
    case 'DealProgressScaled':
      return [
        main([icon('progress'), value(`${effect.base}`, 'progress')]),
        perRider('+', effect.amount, effect.per),
      ]
    case 'DealProgressAll': {
      const lines = [main([icon('progressAll'), value(`${effect.base}`, 'progress'), text('all')])]
      if (effect.bonus) lines.push(bonusRider(effect.bonus))
      return lines
    }
    case 'Draw': {
    ...
    case 'DiscardThenDraw':
      return [main([icon('discard'), value('1'), text('then'), icon('draw'), value(`${effect.player}`)])]
    ...
    case 'Sequence': {
      ...
      return compiledSteps.flatMap(([first, ...rest], index) => {
        ...
        const tokens = index === 0 ? first.tokens : [text('then'), ...first.tokens]
